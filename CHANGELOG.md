...
| 版本 | 日期 | 主要变更 |
|------|------|----------|
| V9.8.1 | 2026-07-09 | **本地工具保存链路改为服务端系统保存窗口**：新增 `preview_player.html` 作为工具 iframe 的纯播放预览，避免 iframe 内下载受浏览器策略影响；工具页外层新增“保存 JSON”按钮，调用 `/save-json` 由本地 Python 服务端弹出系统保存窗口并复制 `merged_output.json`；独立 `preview_embedded.html` 保留 FileSaver + 原生下载兜底，适合双击预览文件时使用；Stage 5 自检同步检查独立预览下载逻辑和工具纯播放预览无下载入口 |
| V9.8.0 | 2026-07-09 | **本地工具工作流与静态视觉标记升级**：工具页改为先渲染静态画面，再在画面中可选标记装饰元素/突出元素，最后生成动效；可选框与实际可动效图层共用识别结果，父子组以父级边界为准，子层视觉框映射回父级动效目标；边界框默认可见，支持边框+范围内双重判定并优化小框选取；生成后静态标记与动态预览解耦，可继续修改标记并重新生成；界面移除技术包装说明和输出名称输入，下载命名放到动效预览面板；预览下载改为原生下载优先、FileSaver 兜底，修复部分环境点击无反应的问题 |
