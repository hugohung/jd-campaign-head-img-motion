...
| 版本 | 日期 | 主要变更 |
|------|------|----------|
| V9.7.1 | 2026-07-07 | **修复预览 `file://` 协议下白屏**：`preview_embedded.html` 将 `lottie.min.js` + `FileSaver.min.js` 内容直接内联到 `<script>` 标签中，避免浏览器 `file://` 安全策略阻止外部本地脚本加载；简化 bootstrap 逻辑，无需等待异步加载即可直接初始化 |
| V9.7.0 | 2026-07-07 | **预览容器自适应尺寸**：`_build_preview_html()` 从 `output` 读取 `w`/`h` 动态计算预览层宽高；宽度 ≤ 800px 用 1x 原始尺寸，超过则 0.5x 缩放；标题显示原尺寸提示 `(原尺寸: W×H)` |
